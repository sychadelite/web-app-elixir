<template>
  <div>
    ...
    <div style="height: 30rem; margin-top: 4rem; overflow: auto;">
      <b-table
        :striped="striped"
        :bordered="bordered"
...
import {getHeader} from '../config.js'
  ...
  export default {
    name: 'TableDataSambungan',
    data() {
      return {
        userDataSambunganAuth: [],
        fields: ['id_data', 'flow_rate', 'total_milli_litres', 'total_litres', 'date_record'],
        // items: [
        //   { id_data: 40, flow_rate: 'Dickerson', total_litres: 'Macdonald' },
        //   { id_data: 21, flow_rate: 'Larsen', total_litres: 'Shaw' },
        //   { id_data: 89, flow_rate: 'Geneva', total_litres: 'Wilson' }
        // ],
        tableVariants: [
          'primary',
          'secondary',
    ...
    methods: {
      getAuthDataSambungan() {
        axios.get('/api/dataSambunganAuth', {headers: getHeader()})
        .then(response => {
            console.log('Authenticated Data Sambungan Object', response)                  
            this.userDataSambunganAuth = response.data.data
        })
      }
    },
    mounted() {
      this.getAuthDataSambungan()
    }
